...
var spaceID = '3m9n1x9kjcai';
var accessToken = 'aad1d5340706429a1de4f8f50e338fc824eeb3fbcbe1bfc59050d31ffd954ddd';
...
$(document).ready(function () {
  ...
  $('.items-container').on('click', '.expandable', function () {
    ...
    expandPortfolioItem($(this), function ($expanded) {

      // After expansion, add in the dynamic article content.
      loadPortfolioArticle($expanded, function () {

        // After we load the content, remove the 'waiting-for-portfolio-content'
        // class, but wait at least half a second.
        setTimeout(function () {
          $expanded.children('.portfolio-item-overlay').removeClass('waiting-for-portfolio-content');
        }, 500);
...
function loadLandingPhoto(client) {
  ...
  // Grab the photo.
  client.getEntries({ 'content_type': 'landingCover' }).then(function (response) {

    // Get URL only if valid request (an image has been submitted to contentful)
    var imageURL = response.items && response.items[0] ? response.items[0].fields.photo.fields.file.url : null;
...
function loadTypingAnimations(client) {
  ...
  client.getEntries({ 'content_type': 'changingHeaderText' }).then(function (response) {
...
function loadSocials(client) {
  ...
  client.getEntries({ 'content_type': 'socialLink' }).then(function (response) {

    response.items.forEach(function (item) {

      $('footer > div.social-media-container').append('<a target="_blank" href="' + item.fields.link + '">\n          <img class="social-icon" src="' + item.fields.icon.fields.file.url + '"\n            alt="' + item.fields.icon.fields.title + '"\n          />\n        </a>');
    });
  });
}
...
function loadSiteMeta(client) {
  ...
  client.getEntries({ 'content_type': 'siteMeta' }).then(function (response) {

    // Exit if couldn't load content from contentful.
    if (!response.items || !response.items[0] || !response.items[0].fields) return console.log("Could not load content. (No site settings / meta entry)");
...
function Article(md) {
  ...
  this.markup = '<div class="portfolio-item-extended-content-container portfolio-item-article">' + parseMarkdown.makeHtml(md) + '</div>';
}
...
function expandPortfolioItem($element, callback) {
  ...
  $overlayElement.one('webkitAnimationEnd mozAnimationEnd oAnimationEnd oanimationend animationend', function () {
    ...
    $innerContent.append('<div class="spinner">\n            <div class="bounce1"></div>\n            <div class="bounce2"></div>\n            <div class="bounce3"></div>\n          </div>');
  ...
  client.getEntries({
    ...
    for (var i = 0; i < entries.items.length; i++) {
      output.push({
        ...
        image: {
          title: entries.items[i].fields.coverImage && entries.items[i].fields.coverImage.title ? entries.items[i].fields.title : null,
          url: entries.items[i].fields.coverImage && entries.items[i].fields.coverImage.fields ? "http:" + entries.items[i].fields.coverImage.fields.file.url : null
        },
        // date: {
        //   start: ,
        //   complet
